...
// Printing only one possible way of placing n queen in a nxn grid.
...
bool solveNQueen(int n , int board[][20] , int i){ // i representing ith row
    ...
    for (int j = 0; j < n; j++)
    {
        ...
        if(canPlace(board,n,i,j)){
            ...
            bool success = solveNQueen(n,board,i+1);// for next row{If neeche wala success hai toh upr wala true return kar de parent ko}
    ...
    // If No possible location is found in the jth columns of ith row
    return false;
...
int main(){
    ...
    bool ans = solveNQueen(n,board,0);
    cout << ans << endl;
